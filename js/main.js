...
window.getStableAnonId = getStableAnonId
...
function buildFeedbackBias(name, todayKin, weather) {
    ...
    return weather.map((cell, i) => {
        ...
        if (score > 0) {
            ...
            attention -= 0.18 * local * s
    activity -= 0.16 * local * s
    pressure += 0.12 * local * s
    conflict += 0.15 * local * s
        }
...
function applyTodayContactsToAttractorField(field, users, dayKey = null) {
    ...
    Object.values(contacts || {}).forEach(item => {
        const a =
    byId.get(item.user_a_id) ||
    users.find(u => String(u.name).trim() === String(item.user_a_name || "").trim())

const b =
    byId.get(item.user_b_id) ||
    users.find(u => String(u.name).trim() === String(item.user_b_name || "").trim())
...
function applyTodayContactsToWeather(weather, users, dayKey = null) {
    if (!Array.isArray(weather) || weather.length !== 260 || !Array.isArray(users)) {
        return weather
    }
    ...
    if (!activePairs.length) return weather

    const byId = new Map()

    users.forEach(u => {
        const id = typeof getUserId === "function" ? getUserId(u.name) : u.name
        if (id) byId.set(id, u)
    })

    const next = weather.map(cell => ({ ...cell }))
...
const b =
    ...
    users.find(u => String(u.name).trim() === String(item.user_b_name || "").trim())
        ...
        ;[a, b].forEach(user => {
            ...
            next[i] = {
                ...cell,
                attention: Math.min(1, Number(cell.attention ?? 0.5) + 0.10),
                activity: Math.min(1, Number(cell.activity ?? 0.5) + 0.06),
                pressure: Math.max(0, Number(cell.pressure ?? 0) - 0.04),
                conflict: Math.max(0, Number(cell.conflict ?? 0) - 0.04)
            }
        })
    })
...
window.applyTodayContactsToWeather = applyTodayContactsToWeather
...
if (typeof window.applyMTOSLang === "function") {
    ...
    try {
        let pyodide = getPyodide(MTOS_GLOBALS)
...
if (!pyodide || typeof pyodide.runPython !== "function") {
    ...
    try {
        ...
        console.error("MTOS init failed before run:", e)
        setStatusText("error")
        return
    }
}
...
if (!pyodide || typeof pyodide.runPython !== "function") {
    console.error("Pyodide is still not ready")
    setStatusText("error")
    return
}
...
const weatherWithFeedback = buildFeedbackBias(name, todayKin, weatherWithDiffusion)
const evolvedWeather = applyTodayContactsToWeather(weatherWithFeedback, runUsers, getCurrentRunDay())
...
let score =
    ...
    score += Number(network.supportRatio ?? 0) * 0.30
score -= Number(network.conflictRatio ?? 0) * 0.35
score += Number(network.meanStrength ?? 0) * 0.20
score += ds.realContacts * 0.12
score += ds.realContactWeight * 0.08
