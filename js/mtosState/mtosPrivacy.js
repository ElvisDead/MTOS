...
export function sanitizeTodayContactsDB(db) {
    ...
    Object.entries(db).forEach(([dayKey, row]) => {
        ...
        Object.values(row).forEach(item => {
            ...
            const aName = String(item.user_a_name || item.a || "").trim();
            const bName = String(item.user_b_name || item.b || "").trim();

            const aId = getStableAnonId(aName || item.user_a_id);
            const bId = getStableAnonId(bName || item.user_b_id);
            ...
            const cleanItem = {
                ...
                expiresAt: Number(item.expiresAt ?? Date.now() + 24 * 60 * 60 * 1000),
                weight: Number(item.weight ?? 1)
            };

            out[dayKey][[aId, bId].sort().join("::")] = cleanItem;

            if (aName && bName) {
                out[dayKey][[aName, bName].sort().join("::")] = cleanItem;
            }
        });
