...
function resolveStableContactId(x) {
    const name = String(x || "").trim()

    if (!name) return ""

    if (typeof window !== "undefined" && typeof window.getStableAnonId === "function") {
...
function resolveStableContactIds(a, b) {
    return {
        aId: resolveStableContactId(a),
        bId: resolveStableContactId(b)
    }
}
...
export function findActiveTodayContactRecord(a, b, deps = {}) {
    const {
        getRelationIds = resolveStableContactIds,
        storageKey = MTOS_TODAY_CONTACTS_KEY,
        ttlMs = MTOS_CONTACT_TTL_MS
    } = deps;
    ...
    const possibleKeys = new Set([
        makePairKey(left, right),
        makePairKey(a, b),
        makePairKey(String(a || "").trim(), String(b || "").trim())
    ]);
    ...
    for (const dayKey of Object.keys(db)) {
        ...
        for (const pairKey of Object.keys(row)) {
            const item = row[pairKey];

            if (possibleKeys.has(pairKey)) {
                return { dayKey, item };
            }

            const itemIdKey = makePairKey(item?.user_a_id, item?.user_b_id);
            const itemNameKey = makePairKey(item?.user_a_name, item?.user_b_name);

            if (possibleKeys.has(itemIdKey) || possibleKeys.has(itemNameKey)) {
                return { dayKey, item };
            }
        }
    }
...
const item = {
    user_a_id: aId,
    user_b_id: bId,
    user_a_name: String(a || "").trim(),
    user_b_name: String(b || "").trim(),
    t: now,
    expiresAt: now + ttlMs,
    weight: 1
};
...
// запись по ID
db[safeDayKey][makePairKey(aId, bId)] = item;

// запись по ИМЕНИ (КЛЮЧЕВОЙ ФИКС)
db[safeDayKey][makePairKey(a, b)] = item;
...
function applyContactBoostToRelations(relations, contacts){

    if(!contacts) return relations

    return relations.map(r => {

        const pairKey = [r.source, r.target].sort().join("::")

        if(contacts[pairKey]){
            return {
                ...r,
                value: Number(r.value) + 0.25 // усиление
            }
        }
